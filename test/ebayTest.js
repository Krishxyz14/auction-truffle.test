...
contract("Ebay",(accounts)=>{
    ...
    it ("it should create an auction",async()=>{
       let auctions;
      ...
      // assert((auction[0].min).toNumber()===auctions.min);
     ...
     it ("should not create an offer if auction does not exist",async()=>{
        ...
        await expectRevert(
         ebay.createOffer(1,{from:buyer1,value: auction.min+10}),"Auction doesn't exist"
        );
     });
     it("should not create an offer if price is too low",async()=>{
        ...
        await expectRevert(
            ...
            "msg.value is grater than the minimum and the best offer"
        );
